...
function findExit(pos: PaperPosition): { price: number; reason: string } | null {
  const direction = directionOf(pos.side);
  const liqPrice = liquidationPrice(pos.side, pos.entry, pos.leverage);
...
export class PaperEngine {
  ...
  openPosition(params: FillParams): { orderId: number; status: string } {
    ...
    if (params.reduceOnly) {
      if (existing) this.reduce(existing, params.qty, price);
    } else if (!existing) {
  ...
  /** Books realized PnL on up to qty of pos; returns the unfilled remainder. */
  private reduce(pos: PaperPosition, qty: number, price: number): number {
    const closeQty = Math.min(pos.qty, qty);
    this.startEquity += (price - pos.entry) * closeQty * directionOf(pos.side);
    pos.qty -= closeQty;
